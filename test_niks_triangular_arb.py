...
def make_bot(prices: dict):
    ...
    bot._trade_count = 0
    bot._last_prices = {}
    bot._last_profit_a = Decimal("0")
    bot._last_profit_b = Decimal("0")
...
def test_route_a_places_correct_orders():
    ...
    bot = make_bot(PROFITABLE_A)
    bot._last_profit_a, _ = bot._compute_profits(PROFITABLE_A)
...
def test_route_b_places_correct_orders():
    ...
    bot = make_bot(PROFITABLE_B)
    _, bot._last_profit_b = bot._compute_profits(PROFITABLE_B)
...
def test_cooldown_blocks_repeat():
    ...
    bot.on_tick()  # immediate second tick — should be blocked
    check("first tick fires", count_after_first == 1, f"got {count_after_first}")
    check("second tick blocked by cooldown", bot._trade_count == 1, f"got {bot._trade_count}")


def test_fetch_prices_returns_none_on_missing():
    print("\n── Test 8: _fetch_prices returns None if any pair missing ──")
    bot = make_bot(LIVE_PRICES)
    # Remove one pair from mock
    bot.market_data_provider.get_price_by_type.side_effect = lambda c, p, t: (
        None if p == "USDT-USD" else LIVE_PRICES.get(p, {}).get("bid" if "Bid" in str(t) else "ask")
    )
    result = bot._fetch_prices()
    check("returns None when a pair has no price", result is None)


def test_status_output():
    print("\n── Test 9: format_status produces readable output ──")
    bot = make_bot(PROFITABLE_A)
    bot._last_prices = PROFITABLE_A
    bot._last_profit_a, bot._last_profit_b = bot._compute_profits(PROFITABLE_A)
